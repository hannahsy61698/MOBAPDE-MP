...
public class Board extends View {
    ...
    public void setMainActivity(Game8 a) {
        activity = a;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if(!Engine.isEnded() && event.getAction() == MotionEvent.ACTION_DOWN) {
            int x = (int) (event.getX() / elementWidth);
            int y = (int) (event.getY() / elementHeight);
            char win = Engine.play(x, y);
            invalidate();
    ...
    private void drawBoard(Canvas canvas) {
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                drawElement(canvas, Engine.getElement(i, j), i, j);
            }
        }
    }
